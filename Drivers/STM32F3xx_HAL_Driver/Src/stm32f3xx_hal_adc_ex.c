	...
	if (HAL_IS_BIT_CLR(hadc->State, HAL_ADC_STATE_ERROR_INTERNAL)
		...
		if (hadc->Init.DiscontinuousConvMode == ENABLE) {
			if (hadc->Init.ContinuousConvMode == DISABLE) {
				...
				SET_BIT(tmpCFGR,
						ADC_CFGR_DISCEN | ADC_CFGR_DISCONTINUOUS_NUM(hadc->Init.NbrOfDiscConversion));
			} else {
		...
		if (hadc->Init.ExternalTrigConv != ADC_SOFTWARE_START) {
			SET_BIT(tmpCFGR,
					ADC_CFGR_EXTSEL_SET(hadc, hadc->Init.ExternalTrigConv) | hadc->Init.ExternalTrigConvEdge);
		}
	...
	if (tmp_hal_status == HAL_OK) {
		...
		if ((ADC_IS_ENABLE(hadc) == RESET)
			...
			CLEAR_BIT(tmpADC_Common->CCR,
					ADC_CCR_CKMODE | ADC_CCR_VBATEN | ADC_CCR_TSEN | ADC_CCR_VREFEN | ADC_CCR_MDMA | ADC_CCR_DMACFG | ADC_CCR_DELAY | ADC_CCR_MULTI);
...
void HAL_ADC_IRQHandler(ADC_HandleTypeDef *hadc) {
			...
			if ((READ_BIT (tmp_cfgr, ADC_CFGR_JAUTO) == RESET)
				...
				if ((tmp_isr & ADC_FLAG_JEOS) == ADC_FLAG_JEOS) {
					...
					if (ADC_NONMULTIMODE_INJ_OR_MULTIMODEMASTER(hadc)) {
						...
						tmp_cfgr_jqm = READ_REG(
								ADC_MASTER_INSTANCE(hadc)->CFGR);
					}
	...
	if (((tmp_isr & ADC_FLAG_OVR) == ADC_FLAG_OVR)
		...
		if (hadc->Init.Overrun == ADC_OVR_DATA_PRESERVED) {
			...
			/* Check DMA configuration, depending on MultiMode set or not */
			if (READ_BIT(tmpADC_Common->CCR,
					ADC_CCR_MULTI) == ADC_MODE_INDEPENDENT) {
				if (HAL_IS_BIT_SET(hadc->Instance->CFGR, ADC_CFGR_DMAEN)) {
					overrun_error = 1U;
				}
			} else {
...
HAL_StatusTypeDef HAL_ADCEx_InjectedStop_IT(ADC_HandleTypeDef *hadc) {
	...
	if (HAL_IS_BIT_CLR(hadc->Instance->CFGR, ADC_CFGR_JAUTO)) {
		...
		if (tmp_hal_status == HAL_OK) {
			...
			__HAL_ADC_DISABLE_IT(hadc,
					(ADC_IT_JEOC | ADC_IT_JEOS | ADC_IT_JQOVF));
	...
	if ((hadc->Init.ScanConvMode == ADC_SCAN_DISABLE)
		...
		if (sConfigInjected->InjectedRank == ADC_INJECTED_RANK_1) {
			...
			if (sConfigInjected->ExternalTrigInjecConv
				...
				SET_BIT(tmp_JSQR_ContextQueueBeingBuilt,
						ADC_JSQR_RK(sConfigInjected->InjectedChannel, ADC_INJECTED_RANK_1) | ADC_JSQR_JEXTSEL_SET(hadc, sConfigInjected->ExternalTrigInjecConv) | sConfigInjected->ExternalTrigInjecConvEdge);
			} else {
				SET_BIT(tmp_JSQR_ContextQueueBeingBuilt,
			...
			MODIFY_REG(hadc->Instance->JSQR,
					ADC_JSQR_JSQ4 | ADC_JSQR_JSQ3 | ADC_JSQR_JSQ2 | ADC_JSQR_JSQ1 | ADC_JSQR_JEXTEN | ADC_JSQR_JEXTSEL | ADC_JSQR_JL,
		...
		if (hadc->InjectionConfig.ChannelCount == 0U) {
			...
			if (sConfigInjected->ExternalTrigInjecConv
				...
				SET_BIT(hadc->InjectionConfig.ContextQueue,
						(sConfigInjected->InjectedNbrOfConversion - 1U) | ADC_JSQR_JEXTSEL_SET(hadc, sConfigInjected->ExternalTrigInjecConv) | sConfigInjected->ExternalTrigInjecConvEdge);
			} else {
				SET_BIT(hadc->InjectionConfig.ContextQueue,
						(sConfigInjected->InjectedNbrOfConversion - 1U));
			}
...
HAL_StatusTypeDef HAL_ADC_AnalogWDGConfig(ADC_HandleTypeDef *hadc,
	...
	if (ADC_IS_CONVERSION_ONGOING_REGULAR_INJECTED(hadc) == RESET) {
		...
		if (AnalogWDGConfig->WatchdogNumber == ADC_ANALOGWATCHDOG_1) {
			...
			MODIFY_REG(hadc->Instance->CFGR,
					ADC_CFGR_AWD1SGL | ADC_CFGR_JAWD1EN | ADC_CFGR_AWD1EN | ADC_CFGR_AWD1CH,
					AnalogWDGConfig->WatchdogMode | ADC_CFGR_AWD1CH_SHIFT(AnalogWDGConfig->Channel));
...
HAL_StatusTypeDef HAL_ADCEx_MultiModeConfigChannel(ADC_HandleTypeDef *hadc,
	...
	if ((ADC_IS_CONVERSION_ONGOING_REGULAR(hadc) == RESET)
			&& (ADC_IS_CONVERSION_ONGOING_REGULAR(
					&tmphadcSharingSameCommonRegister) == RESET)) {
		/* Pointer to the common control register to which is belonging hadc      */
		/* (Depending on STM32F3 product, there may have up to 4 ADC and 2 common */
		/* control registers)                                                     */
		tmpADC_Common = ADC_COMMON_REGISTER(hadc);
...
static HAL_StatusTypeDef ADC_ConversionStop(ADC_HandleTypeDef *hadc,
	...
	if (ADC_IS_CONVERSION_ONGOING_REGULAR_INJECTED(hadc)) {
		...
		while ((hadc->Instance->CR & tmp_ADC_CR_ADSTART_JADSTART) != RESET) {
			if ((HAL_GetTick() - tickstart) > ADC_STOP_CONVERSION_TIMEOUT) {
				/* New check to avoid false timeout detection in case of preemption */
				if ((hadc->Instance->CR & tmp_ADC_CR_ADSTART_JADSTART)
						!= RESET) {
					/* Update ADC state machine to error */
					SET_BIT(hadc->State, HAL_ADC_STATE_ERROR_INTERNAL);
