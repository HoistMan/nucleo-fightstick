...
#define __LL_ADC_CALC_TEMPERATURE_TYP_PARAMS(__TEMPSENSOR_TYP_AVGSLOPE__,\
  ...
  ((( (                                                                        \
	...
	__STATIC_INLINE void LL_ADC_SetOffsetState(ADC_TypeDef *ADCx,
		...
		__IO uint32_t *preg =
				(__IO uint32_t*) ((uint32_t) ((uint32_t) (&ADCx->OFR1)
						+ (Offsety * 4U)));
	...
	__STATIC_INLINE uint32_t LL_ADC_REG_GetTriggerSource(ADC_TypeDef *ADCx) {
		uint32_t TriggerSource = READ_BIT(ADCx->CFGR,
	...
	__STATIC_INLINE uint32_t LL_ADC_INJ_GetTriggerSource(ADC_TypeDef *ADCx) {
		uint32_t TriggerSource = READ_BIT(ADCx->JSQR,
	...
	__STATIC_INLINE void LL_ADC_SetAnalogWDMonitChannels(ADC_TypeDef *ADCx,
		...
		__IO uint32_t *preg =
				__ADC_PTR_REG_OFFSET(ADCx->CFGR,
						__ADC_MASK_SHIFT(AWDy, ADC_AWD_CRX_REGOFFSET_MASK) + ((AWDy & ADC_AWD_CR12_REGOFFSETGAP_MASK) * ADC_AWD_CR12_REGOFFSETGAP_VAL));
	...
	__STATIC_INLINE uint32_t LL_ADC_GetAnalogWDMonitChannels(ADC_TypeDef *ADCx,
		...
		uint32_t AWD123ChannelGroup = READ_BIT(*preg,
		...
		/*       conflict.                                                          */
		uint32_t AWD23Enabled = ((((uint32_t) 0x00000001U)
				>> (AWD123ChannelGroup % 3U)) << 6U); /* Value "0" if AWD2-3 is enabled, value "32" if AWD2-3 is disabled */
						...
						| ((ADC_CFGR_AWD1SGL)
								>> ((((uint32_t) 0x00000001U)
										>> (ADC_AWD_CR23_CHANNEL_MASK
												- AWD123ChannelGroup)) << 5U))
						| (ADC_CFGR_JAWD1EN | ADC_CFGR_AWD1EN)) >> AWD23Enabled)
						>> (((AWDy & ADC_CFGR_AWD1SGL)
								>> ADC_CFGR_AWD1SGL_BITOFFSET_POS) << 5U));
	...
	__STATIC_INLINE void LL_ADC_ConfigAnalogWDThresholds(ADC_TypeDef *ADCx,
		...
		__IO uint32_t *preg = __ADC_PTR_REG_OFFSET(ADCx->TR1,
				__ADC_MASK_SHIFT(AWDy, ADC_AWD_TRX_REGOFFSET_MASK));
	...
	__STATIC_INLINE uint32_t LL_ADC_REG_ReadMultiConversionData32(
		...
		return (uint32_t) (READ_BIT(ADCxy_COMMON->CDR,
				ConversionData) >> POSITION_VAL(ConversionData));
	}
#endif /* ADC_MULTIMODE_SUPPORT */
